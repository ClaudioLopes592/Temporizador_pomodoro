...
var bell = new Audio('../static/audio/audio_bell.mp3')
var volta = new Audio('../static/audio/audio_volta.mp3')
var final = new Audio('../static/audio/audio_final.mp3')
...
/**Adicionando focus no campo ação*/
acao.focus()
...
function momentoAcao() {
    ...
    // Muda a expessura do texto para bold
    ...
    // Adiciona a variável min_interval a função setInterval que vai executar a função minTimer de 60 em 60 segundos
    var min_interval = setInterval(minTimer, 60000)
    // Adiciona a variável seg_interval a função setInterval que vai executar a função segTimer de 1 em 1 segundo
...
function momentoPausa() {
    ...
    function segTimer() {
        ...
        // Verificação se o valor de segundos é menor ou igual a 0
        if (segundos <= 0) {
            // Verificação se o valor de min_pausa é menor ou igual a 0  
            if (min_pausa <= 0) {
                // Se acabar os minutos, pega o valor de sessões que está no localStorage já convertendo para number e adicionando a variável sess
                sess = Number(localStorage.getItem('sessoes'))
